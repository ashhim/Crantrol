...
<body>
  ...
  <script>
    let autoSequenceBusy = false;
    function relayDefaults(id) {
      const defaults = {
        1: { id: 1, name: 'PLUG1', pin: 21, activeLow: true, pulseMs: 350, pulseDurationMs: 350, enabled: true, isPulse: false, state: false, isOn: false },
        2: { id: 2, name: 'PLUG2', pin: 17, activeLow: true, pulseMs: 0, pulseDurationMs: 0, enabled: true, isPulse: false, state: false, isOn: false },
        3: { id: 3, name: 'PLUG3', pin: 16, activeLow: true, pulseMs: 0, pulseDurationMs: 0, enabled: true, isPulse: false, state: false, isOn: false },
        4: { id: 4, name: 'PLUG4', pin: 18, activeLow: true, pulseMs: 250, pulseDurationMs: 250, enabled: true, isPulse: false, state: false, isOn: false },
        5: { id: 5, name: 'PLUG5', pin: 15, activeLow: true, pulseMs: 0, pulseDurationMs: 0, enabled: true, isPulse: false, state: false, isOn: false },
        6: { id: 6, name: 'PLUG6', pin: 3, activeLow: true, pulseMs: 0, pulseDurationMs: 0, enabled: true, isPulse: false, state: false, isOn: false },
        7: { id: 7, name: 'PLUG7', pin: 2, activeLow: true, pulseMs: 0, pulseDurationMs: 0, enabled: true, isPulse: false, state: false, isOn: false },
        8: { id: 8, name: 'PLUG8', pin: 1, activeLow: true, pulseMs: 0, pulseDurationMs: 0, enabled: true, isPulse: false, state: false, isOn: false },
    ...
    async function updateStatus() {
      try {
        ...
        relayGrid.innerHTML = relaysArray.map(relay => {
          ...
          return `
            <div class="relay-item-card ${isOn ? 'on' : ''}" style="display: flex; flex-direction: column; align-items: center; justify-content: space-between; min-height: 132px; padding: 12px; opacity: ${isEnabled ? '1' : '0.72'};">
              <strong>${label}</strong>
      ...
      card.innerHTML = `
        ...
        <div class="form-row">
          ...
          <div class="form-group checkbox-group" style="margin-top:25px;">
            ...
            <label for="enabled-${relay.id}">${relay.id <= 8 ? 'Visible in Dashboard' : 'Enabled'}</label>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group checkbox-group" style="margin-top:25px;">
