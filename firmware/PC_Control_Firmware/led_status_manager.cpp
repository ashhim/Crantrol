...
void LedStatusManager::update() {
  ...
  if (buzzerPin > 0) {
    if (buzzerManualOn) {
      // ALERT hold: continuous beep while held
      tone(buzzerPin, 2200);
...
void LedStatusManager::triggerBlinkFeedback() {
  feedbackEndTime = millis() + 500; // Visual flash for 500 milliseconds
  buzzerFeedbackEndTime = millis() + 1000; // One-second buzzer pulse
}
